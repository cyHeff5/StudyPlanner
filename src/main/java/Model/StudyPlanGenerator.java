...
public class StudyPlanGenerator {
     ...
     */
    public StudyPlanGenerator(List<Event> events, List<Modul> modules, com.calendarfx.model.Calendar studyPlan, EntityManager entityManager, EntityTransaction entityTransaction) {
        this.events = events;
    ...
    /**
     * calculates how many filler Events every Module gets assigned to, based on the ECTS amount.
    ...
    /**
     * Assigns the amount of fillerEvents to Modules, that haven't been assigned yet.
     *
    ...
    private void generateStudyPlan() {
        ...
        for(Event event : getFillerEvents()) {
            ...
            List<Event> removeEvents = events.stream().filter(e -> e.getId().equals(event.getId())).collect(Collectors.toList());
